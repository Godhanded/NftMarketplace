...
export default function UpdateListingModal({nftAddress,tokenId,isVisible,onClose}){
     ...
     const {error: prepareError,config,isError:isPrepareError}=usePrepareContractWrite({
    ...
    return (
        <Modal isVisible={isVisible} onOk={()=>write?.()} isOkDisabled={(!write || isLoading)} onCancel={onClose} onCloseButtonPressed={onClose}>
            <Input 
            ...
            {(isPrepareError || isError) && <span className="italic text-red-400">Error {(error || prepareError)?.message}</span>}
        </Modal>
    )
} 